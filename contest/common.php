...
function openContest($idItem, $idUser, $idGroupSelf, $reopen = false) {
	...
	$stmt = $db->prepare('
        select NOW() as now, items.*, users_items.*, TIME_TO_SEC(items.sDuration) as duration, max(groups_items.bCachedFullAccess) as fullAccess, DATE_ADD(items.sEndContestDate, INTERVAL 1 DAY) as contestEndDate
        from items
		left join users_items on users_items.idItem = items.ID and users_items.idUser = :idUser
